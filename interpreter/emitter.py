...
def _terminators(insts: List[Instruction]) -> Set[str]:
    return {
        ...
        for inst in insts
        if any(
            isinstance(node, EnvExec)
            and isinstance(node.interface, (SetPC, SysCall))
...
class InterpEmitter:
    ...
    def isa(self, insts: List[Instruction]) -> str:
        ...
        ilens = {
            inst.name: inst.lira_insn.encoding.encoded_size // 8
            for inst in insts
        }
        return self.cfg.templates.render(
            self.cfg.isa_hh_jinja, insts=insts,
            terminators=terminators, ilens=ilens,
    ...
    def decoder(self, index, insts: List[Instruction]) -> Tuple[str, str]:
        return (
            ...
            self.cfg.templates.render(self.cfg.decoder_cc_jinja,
                                      insts=insts),
        )
    ...
    def interpreter(self, insts: List[Instruction]) -> Tuple[str, str]:
        ...
        return (
            ...
            self.cfg.templates.render(
                self.cfg.interpreter_cc_jinja, insts=insts,
                bodies=bodies,
                cpu_var=self.cfg.CPU_VAR, mem_var=self.cfg.MEM_VAR,
            ),
        )

    def cpu_state(self, arch, interfaces, reg_files) -> Tuple[str, str]:
        rf_code = "\n".join(
            reg_files[rf.name].definition
        ...
        return (
            self.cfg.templates.render(self.cfg.cpu_state_hh_jinja,
                                      rf_code=rf_code, decls=decls),
            self.cfg.templates.render(self.cfg.cpu_state_cc_jinja,
                                      defs=defs),
        )
    ...
    def base_ops(self, operations) -> Tuple[str, str]:
        ...
        return (
            self.cfg.templates.render(self.cfg.base_ops_hh_jinja,
                                      decls=decls),
            self.cfg.templates.render(self.cfg.base_ops_cc_jinja,
                                      defs=defs),
        )
    ...
    def snippets(self, funcs) -> Tuple[str, str]:
        ...
        return (
            self.cfg.templates.render(self.cfg.snippets_hh_jinja,
                                      decls=decls),
            self.cfg.templates.render(self.cfg.snippets_cc_jinja,
                                      defs=defs),
        )
